...
"    float3 edge2 = p2 - p0;\n"
"    \n"
"    // Длина векторного произведения дает площадь параллелограмма\n"
"    // Делим на 2, чтобы получить площадь треугольника\n"
...
"{\n"
"    // Находим два вектора из одной точки\n"
"    float3 edge1 = p1 - p0;\n"
"    float3 edge2 = p2 - p0;\n"
"    \n"
"    // Вычисляем векторное произведение и нормализуем результат\n"
"    // Порядок (edge1, edge2) определяет направление нормали (правило правой руки)\n"
...
"            float3 sel2 = float3(.225,.4,.5);\n"
"          \n"
"            float  col_i = saturate(pow(saturate(.025/length(tp-sel)),7)*1111122);\n"
"            float4 eyes = 1;\n"
"            eyes = col_i*(.5+.5*sin(atan(normalize(tp.xy-sel2.xy))*24.+time.x)).xxxx*10;\n"
"            eyes = col_i*(sin(col_i*9+5.1)-1+col_i)*12;\n"
"            p1.color= lerp(p1.color,eyes,col_i);\n"
"\n"
"    }\n"
"\n"
"    if (mode==2)\n"
...
"    float phi=p.z;\n"
"    float3 cartesian;\n"
"    // Р’ HLSL РѕР±С‹С‡РЅРѕ Y вЂ” СЌС‚Рѕ РІРµСЂС… (Up), РїРѕСЌС‚РѕРјСѓ:\n"
"    // x = r * sin(phi) * cos(theta)\n"
"    // y = r * cos(phi)\n"
"    // z = r * sin(phi) * sin(theta)\n"
"    \n"
...
"    float radius = length(p);\n"
"    \n"
"    // Р•СЃР»Рё С‚РѕС‡РєР° РІ С†РµРЅС‚СЂРµ РєРѕРѕСЂРґРёРЅР°С‚, СѓРіР»С‹ РЅРµ РѕРїСЂРµРґРµР»РµРЅС‹\n"
"    if (radius <= 0.0) return float3(0, 0, 0);\n"
"\n"
"    // theta = СѓРіРѕР» РІ РїР»РѕСЃРєРѕСЃС‚Рё XZ (Р°Р·РёРјСѓС‚)\n"
"    // РСЃРїРѕР»СЊР·СѓРµРј atan2(z, x) РґР»СЏ РїРѕР»РЅРѕРіРѕ РґРёР°РїР°Р·РѕРЅР° [-PI, PI]\n"
"    float theta = atan2(p.z, p.x);\n"
"    \n"
"    // phi = СѓРіРѕР» РѕС‚ РѕСЃРё Y (Р·РµРЅРёС‚) [0, PI]\n"
"    // acos РІРѕР·РІСЂР°С‰Р°РµС‚ Р·РЅР°С‡РµРЅРёСЏ РѕС‚ 0 РґРѕ PI\n"
"    float phi = acos(p.y / radius);\n"
"    \n"
"    return float3(radius, theta, phi);\n"
"}\n"
