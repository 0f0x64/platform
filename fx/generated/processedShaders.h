...
"    pos.xz*=.7+max(-pos.y,.0)*.001;\n"
"    pos.y*=1-max(-pos.y+noise(pos)*222,0)*.01;\n"
"    //pos=lerp(pos,normalize(pos-float3(0,121,0))*112,.0);\n"
"    //pos=rotY(pos,-111111/pow(length(pos),3));\n"
"    pos.y+=-1111/pow(length(pos)*.1,3);\n"
"\n"
"    //star\n"
"    float f=1-saturate(length(pos+float3(0,-30,0))*.00351*(noise(pos)+1.2));\n"
"    f=step(.5,pow(f,2));\n"
"\n"
"    pos = lerp(pos,normalize(noise3(pos))*26*(noise(pos*2)+1),f);\n"
"\n"
"    pos*=1+noise(pos*.1)*f*55;\n"
"    \n"
"    float cf=195/(length(pos)+1)*1;\n"
"\n"
...
"\n"
"    //pos.y*=1+(length(pos.xz)<2)*11*f;\n"
"    if (!(st||br||br2)) \n"
"    {\n"
"        pos.y*=1+(12/(length(pow(pos.xz,5))*.0001+1))*f*2.2;\n"
"        pos.y*=1-.5*f;\n"
"    }\n"
"    pos*=1-f/1.3;\n"
"\n"
...
"    posT.xy+=(grid.zw-.5)*br2*15.5*pow(sin(qid)*.5+.55,.25)*scale*2*max(pow(length(pos2),1.28)*.00003,1)*1.2;\n"
"    posT.xy+=(1-st)*(1+1*sin(qid*.01))*gzw*posT.w/250*(1-st);\n"
"    posT.xy+=(gzw)*1.2*posT.w/300*st;\n"
"\n"
