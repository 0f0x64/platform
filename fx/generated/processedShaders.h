...
"\n"
"    float2 uvsR = rot(uvs,progress_radial.x*3.14*2);\n"
"    float ang = 1.-18*abs(uvsR.x);\n"
"    float progressR = ang*saturate(-uvsR.y);\n"
"    //float progressR = progress_radial < ang ? 0. : .125;\n"
"    //progressR+=progress_radial > 0 ? length(uvs)*saturate(pow(saturate(1-2*abs(ang-progress_radial)),28)) :0;\n"
"    float signedProgressRad = saturate(pow(saturate(1-2*abs(ang-progress_radial)),8));\n"
"   // signedProgressRad*=4*pow(length(uvs)+1,2);\n"
"    //progressR=lerp(progressR,signedProgressRad,signed_progress);\n"
"   \n"
"\n"
