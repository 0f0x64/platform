...
"    float3 rightDir = normalize(cross(worldViewDir, localUp));\n"
"    float3 upDir = cross(rightDir, worldViewDir);\n"
"\n"
"    float radialX = dot(p, rightDir);\n"
"    float radialY = dot(p, upDir);\n"
"\n"
"    float3 viewRight = normalize(cross(worldViewDir, float3(0, 1, 0)));\n"
"    float3 viewUp    = cross(viewRight, worldViewDir);\n"
"\n"
"    // Считаем угол строго по направлению изменения вектора взгляда камеры\n"
"    float phi = atan2(dot(worldViewDir, viewUp), dot(worldViewDir, viewRight));\n"
"\n"
"    //float phi = atan2(radialY, radialX);\n"
"    \n"
"    // Мягкая радиальная развертка: Y идет плавно от центра наружу\n"
"    float2 radialUV = float2(phi / 6.2831/22222, 1.0 - edgeFade);\n"
"\n"
"    // =================================================================\n"
"    // ЗОНИРОВАНИЕ (Теперь мягкое, без резких ступеней)\n"
"    // =================================================================\n"
"    // Базовый диск плавно зануляется к краям\n"
"    float diskZone = saturate(edgeFade * 2.0 - 0.4); \n"
"    \n"
...
"    // =================================================================\n"
"    float3 p_disk = p + rot3(p, noise3(p * 26.41 + t * 0.5)) * 0.12;\n"
"\n"
"    float cellTex = saturate(0.15 / cells3D(p_disk * 1.4) * 0.8) * diskZone;\n"
"    \n"
"    // =================================================================\n"
...
"    float radialNoise1 = noise3(float3(radialUV.x * 145.0, radialUV.y * 14.0, t * .10)) * 0.5 + 0.5;\n"
"    float radialNoise2 = noise3(float3(radialUV.x * 165.0, radialUV.y * 16.0, -t * 0.3)) * 0.5 + 0.5;\n"
"    \n"
"    // МЯГКОЕ смешивание вместо жесткого pow(..., 2.2). Лучи получаются пушистыми\n"
"    float coronaTex = saturate((radialNoise1 + radialNoise2) * 0.5) * coronaZone;\n"
"    \n"
"    // Плавное затухание короны к краям геометрии (умножаем на edgeFade)\n"
"    coronaTex *= edgeFade * 1.5;\n"
"    coronaTex *= 1-saturate(diskZone*3);\n"
"    //coronaTex *=pow(cells3D(p),3)+.5;\n"
"    // =================================================================\n"
"    // СМЕШИВАНИЕ И МЯГКИЙ РАСЧЕТ ЦВЕТА\n"
"    // =================================================================\n"
"    // Соединяем текстуры. Диск дает плотность в центре, корона — пушистость на краях\n"
"    float finalPlasma = (cellTex * 1112) + (coronaTex * 4.0);\n"
"\n"
"    // Градиент цвета: мягкий переход от оранжевого к дымчатому бело-желтому свечению\n"
"    float3 centerColor = float3(1.0, 0.25, 0.05) * finalPlasma;\n"
"    float3 edgeGlow    = float3(2.0, 1.7, 1.3) * coronaZone * finalPlasma * 13.5;\n"
"    \n"
"    float3 finalColor = (centerColor + edgeGlow) * 2222.0 * input.color.rgb;\n"
"\n"
"    float coronaMask = saturate(pow(edgeFade,2));\n"
"    coronaMask=pow(.5+.5*cos(edgeFade*PI-.81),231)*15;\n"
"    coronaMask*=1-saturate(diskZone);\n"
"    finalColor+=pow(coronaMask,1)*22222*(input.color+1);\n"
"\n"
"    // =================================================================\n"
"    // РВАНЫЙ КРАЙ БЕЗ ГРЯЗИ\n"
"    // =================================================================\n"
"    // Возвращаем вашу исходную мягкую альфу для таяния краев\n"
"    float alphaMask = diskZone + coronaTex * 3.0;\n"
