...
"\n"
"    \n"
"    float br2 = (qid%316*div) ==  0  ? 1 : 0;\n"
"    float st = (qid%3206*div) ==  0  ? 1 : 0;\n"
"    float br = (qid%200000*div) ==  0  ? 1 : 0;\n"
"    output.pos.xy+=(grid.zw-.5)*br*232.5*scale;\n"
"    output.pos.xy+=(grid.zw-.5)*br2*15.5*pow(sin(qid)*.5+.55,.25)*scale*2.;\n"
...
"    float3 n =.125*(abs(noise3(l1-l2)*float3(6,2,4))+.95);\n"
"    float4 neb = float4(n,1);\n"
"    output.rgba = neb;\n"
"    output.rgba = br2 ? .02: output.rgba ;\n"
"    output.rgba = (st ? 14*(1-br)*output.rgba : output.rgba);\n"
"    output.rgba = br ? float4(3.95,2,4,1)*.05 : output.rgba;\n"
"    \n"
"  \n"
"    if (!st&!br2&!br) output.rgba/=output.pos.w*.01;\n"
"    output.rgba*=saturate(output.pos.z*.001+.4)/pow(div,.5);\n"
"    \n"
