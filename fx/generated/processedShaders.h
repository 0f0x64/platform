...
"    Progress+=progress>0 ? abs(uvs.x)*saturate(pow(saturate(1-2*abs(iuv-progress)),28)) :0;\n"
...
"    if (slider_type==3)\n"
"    {\n"
"    float2 uvsR = rot(uvs,progress.x*3.14*3/2);\n"
"    float ang = saturate(1.-18*abs(uvsR.x));\n"
"    Progress = ang*saturate(sign(-uvsR.y-.25));\n"
"    Progress += saturate(1-18*abs(length(uvs)-.45));\n"
"    Progress *= saturate(1-2*length(uvs));\n"
"    Progress = saturate(Progress*3);\n"
"    signedProgress = saturate(pow(saturate(1-2*abs(ang-progress)),8));\n"
"    signedProgress*=4*pow(abs(ang),2);\n"
"    \n"
"    }\n"
"  \n"
"    \n"
