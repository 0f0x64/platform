...
cbuffer params:register(b0){float4x4 model;int gX;int gY;int mode;int skipper;float4 base_color;float4 mesh[40];};float toRad(float p){return p*PI/180.;}float smin(float g,float p,float m){float s=clamp(.5+.5*(p-g)/m,0.,1.);return lerp(p,g,s)-m*s*(1.-s);}float3 hash33(float3 p){return float3(hash(p.x),hash(p.y),hash(p.z))-.5;}float3 hash31(float p){float3 g=frac(p*float3(.1031,.103,.0973));g+=dot(g,g.yzx+33.33);return frac((g.xxy+g.yzz)*g.zyx);}float3 sm2(float g){return smoothstep(0,1,smoothstep(0,1,g));}float3 sm3(float3 g){return smoothstep(0,1,smoothstep(0,1,g));}float3 getRandomPointInTriangle(float3 g,float3 p,float3 s,float i){float2 f;f.x=hash(i);f.y=hash(f.x);f.y=f.x/3;i=sqrt(f.x);float n=1.f-i;i*=f.y;return n*g+i*p+(1.f-n-i)*s;}pos_color CalcParticles(uint qid,uint iid,float4 grid)
{
