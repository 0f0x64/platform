...
// 8. ФИНИШ B
//
// X — основное направление движения
// Y — высота
// Z — глубина
//
// Все линии заданы вручную точками.
// Никаких sin/cos/генераторов.
// ============================================================
...
// СТАРТОВАЯ ПЛАТФОРМА
		NewLine();
		AddPointToLine({ 10000, 50000, 50000 });
		AddPointToLine({ 11000, 50000, 50000 });
		AddPointToLine({ 12000, 50000, 50000 });
		AddPointToLine({ 13000, 50000, 50000 });


		// ============================================================
		// 01 — РАЗГОН
		// Игрок сначала должен быстро пройти длинный участок.
		// ============================================================

		NewLine();
		AddPointToLine({ 13500, 50000, 50000 });
		AddPointToLine({ 14500, 50100, 50200 });
		AddPointToLine({ 15500, 50300, 50500 });
		AddPointToLine({ 16500, 50600, 50700 });
		AddPointToLine({ 17500, 50900, 50500 });
		AddPointToLine({ 18500, 51100, 50200 });
		AddPointToLine({ 19500, 51000, 50000 });
		AddPointToLine({ 20500, 50700, 49800 });


		// ============================================================
		// 02 — РЕЗКОЕ ТОРМОЖЕНИЕ
		// Платформа становится короче и уходит вверх.
		// Нужно сбросить скорость перед прыжком.
		// ============================================================

		NewLine();
		AddPointToLine({ 21000, 50700, 49800 });
		AddPointToLine({ 21500, 50300, 50000 });
		AddPointToLine({ 22000, 49800, 50300 });
		AddPointToLine({ 22500, 49400, 50500 });
		AddPointToLine({ 23000, 49600, 50300 });


		// ============================================================
		// 03 — ПЕРВЫЙ РАЗРЫВ
		// Небольшая платформа перед первым прыжком.
		// ============================================================

		NewLine();
		AddPointToLine({ 23500, 49600, 50300 });
		AddPointToLine({ 24000, 49800, 50600 });
		AddPointToLine({ 24500, 50000, 50800 });
		AddPointToLine({ 25000, 49900, 50600 });


		// ============================================================
		// 04 — ПЕРВЫЙ ПРЫЖОК
		// Разрыв между платформами.
		// Вторая платформа выше.
		// ============================================================

		NewLine();
		AddPointToLine({ 25800, 50500, 51000 });
		AddPointToLine({ 26500, 51200, 51300 });
		AddPointToLine({ 27200, 51800, 51100 });
		AddPointToLine({ 27900, 52000, 50700 });
		AddPointToLine({ 28600, 51600, 50400 });


		// ============================================================
		// 05 — УЗКОЕ МЕСТО
		// После прыжка нужно точно приземлиться
		// и удержать движение по узкой платформе.
		// ============================================================

		NewLine();
		AddPointToLine({ 28900, 51600, 50400 });
		AddPointToLine({ 29500, 51700, 50500 });
		AddPointToLine({ 30100, 51600, 50700 });
		AddPointToLine({ 30700, 51400, 50500 });
		AddPointToLine({ 31300, 51100, 50200 });


		// ============================================================
		// 06 — УЗКАЯ ВОЛНА
		// Платформа начинает резко менять направление по Y.
		// ============================================================

		NewLine();
		AddPointToLine({ 31700, 51000, 50200 });
		AddPointToLine({ 32200, 50500, 50400 });
		AddPointToLine({ 32700, 49900, 50700 });
		AddPointToLine({ 33200, 49400, 50900 });
		AddPointToLine({ 33700, 49200, 50700 });
		AddPointToLine({ 34200, 49600, 50400 });
		AddPointToLine({ 34700, 50200, 50100 });


		// ============================================================
		// 07 — РЕЗКИЙ ПОДЪЁМ
		// Нужно изменить вертикальное движение,
		// не теряя скорость.
		// ============================================================

		NewLine();
		AddPointToLine({ 35100, 50300, 50100 });
		AddPointToLine({ 35600, 50800, 50400 });
		AddPointToLine({ 36100, 51500, 50800 });
		AddPointToLine({ 36600, 52200, 51100 });
		AddPointToLine({ 37100, 52800, 50900 });
		AddPointToLine({ 37600, 53100, 50500 });


		// ============================================================
		// 08 — МАЛЕНЬКАЯ ПЛАТФОРМА
		// Точка подготовки перед вторым прыжком.
		// ============================================================

		NewLine();
		AddPointToLine({ 38000, 53100, 50500 });
		AddPointToLine({ 38500, 52900, 50300 });
		AddPointToLine({ 39000, 52600, 50200 });
		AddPointToLine({ 39500, 52700, 50400 });


		// ============================================================
		// 09 — ВТОРОЙ ПРЫЖОК
		// Более длинный разрыв.
		// Целевая платформа ниже.
		// ============================================================

		NewLine();
		AddPointToLine({ 40400, 52000, 50700 });
		AddPointToLine({ 41200, 51300, 51000 });
		AddPointToLine({ 42000, 50700, 50800 });
		AddPointToLine({ 42800, 50400, 50400 });
		AddPointToLine({ 43600, 50700, 50100 });
		AddPointToLine({ 44400, 51200, 50300 });


		// ============================================================
		// 10 — УСКОРЕНИЕ
		// После приземления нужно снова набрать скорость.
		// ============================================================

		NewLine();
		AddPointToLine({ 44800, 51200, 50300 });
		AddPointToLine({ 45600, 50900, 50500 });
		AddPointToLine({ 46400, 50400, 50800 });
		AddPointToLine({ 47200, 49800, 51000 });
		AddPointToLine({ 48000, 49600, 50700 });
		AddPointToLine({ 48800, 50000, 50300 });
		AddPointToLine({ 49600, 50600, 50000 });
		AddPointToLine({ 50400, 50900, 50200 });


		// ============================================================
		// 11 — РЕЗКИЙ СЛОМ НАПРАВЛЕНИЯ
		// Игрок должен не просто продолжать движение,
		// а быстро изменить направление.
		// ============================================================

		NewLine();
		AddPointToLine({ 50800, 50900, 50200 });
		AddPointToLine({ 51400, 50500, 50500 });
		AddPointToLine({ 51900, 49900, 50800 });
		AddPointToLine({ 52200, 49300, 51000 });
		AddPointToLine({ 52000, 48800, 50700 });
		AddPointToLine({ 51500, 48600, 50300 });
		AddPointToLine({ 51000, 48900, 50000 });


		// ============================================================
		// 12 — РАЗВОРОТ
		// Короткая U-образная секция.
		// Здесь направление движения меняется почти на 180°.
		// ============================================================

		NewLine();
		AddPointToLine({ 50800, 48900, 50000 });
		AddPointToLine({ 50300, 48400, 50200 });
		AddPointToLine({ 49700, 48000, 50500 });
		AddPointToLine({ 49100, 47900, 50800 });
		AddPointToLine({ 48600, 48300, 50500 });
		AddPointToLine({ 48200, 48800, 50200 });
		AddPointToLine({ 48000, 49300, 49900 });


		// ============================================================
		// 13 — ОБРАТНЫЙ РАЗГОН
		// Теперь движение идёт обратно по X.
		// ============================================================

		NewLine();
		AddPointToLine({ 47500, 49400, 49900 });
		AddPointToLine({ 46600, 49100, 50100 });
		AddPointToLine({ 45700, 48600, 50400 });
		AddPointToLine({ 44800, 48200, 50700 });
		AddPointToLine({ 43900, 48400, 50900 });
		AddPointToLine({ 43000, 49000, 50600 });
		...
		// ПЕРВЫЙ
		NewLine();
		AddPointToLine({ 42500, 49100, 50600 });
		AddPointToLine({ 41900, 49400, 50400 });
		AddPointToLine({ 41300, 49300, 50100 });


		// ВТОРОЙ
		NewLine();
		AddPointToLine({ 40600, 48900, 50200 });
		AddPointToLine({ 40000, 48500, 50500 });
		AddPointToLine({ 39400, 48700, 50800 });


		// ТРЕТИЙ
		NewLine();
		AddPointToLine({ 38700, 49200, 50700 });
		AddPointToLine({ 38100, 49600, 50400 });
		AddPointToLine({ 37500, 49400, 50100 });


		// ============================================================
		// 15 — НИЗКАЯ ПЛАТФОРМА
		// Нужно быстро приземлиться и сразу продолжить.
		// ============================================================

		NewLine();
		AddPointToLine({ 36800, 49000, 50000 });
		AddPointToLine({ 36100, 48600, 50300 });
		AddPointToLine({ 35400, 48400, 50600 });
		AddPointToLine({ 34700, 48700, 50800 });
		AddPointToLine({ 34000, 49200, 50500 });
		...
		NewLine();
		AddPointToLine({ 33300, 49200, 50500 });
		AddPointToLine({ 32700, 49600, 50800 });
		AddPointToLine({ 32100, 50200, 51100 });
		AddPointToLine({ 31500, 50900, 51300 });
		AddPointToLine({ 30900, 51500, 51000 });
		AddPointToLine({ 30300, 51800, 50600 });
		...
		NewLine();
		AddPointToLine({ 29500, 51800, 50600 });
		AddPointToLine({ 28700, 51400, 50900 });
		AddPointToLine({ 27900, 50700, 51100 });
		AddPointToLine({ 27100, 50100, 50800 });
		AddPointToLine({ 26300, 49800, 50400 });
		...
		NewLine();
		AddPointToLine({ 25800, 49800, 50400 });
		AddPointToLine({ 25000, 50200, 50200 });
		AddPointToLine({ 24200, 50700, 50000 });
		AddPointToLine({ 23400, 51000, 50200 });
		AddPointToLine({ 22600, 50800, 50500 });
		AddPointToLine({ 21800, 50300, 50300 });
		...
		NewLine();
		AddPointToLine({ 21200, 50200, 50300 });
		AddPointToLine({ 20500, 50000, 50100 });
		AddPointToLine({ 19800, 50100, 50000 });
		AddPointToLine({ 19100, 50400, 50000 });
